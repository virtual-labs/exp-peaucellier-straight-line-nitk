...
function varchange() {
  ...
  // monitoring change in value and connecting slider and number
  // setting trace point coordinate arrays to empty on change of link length
  $("#QCslider").on("slide", function (e, ui) {
    $("#QCspinner").spinner("value", ui.value);
    ptx = [];
    pty = [];
  });
  $("#QCspinner").on("spin", function (e, ui) {
    $("#QCslider").slider("value", ui.value);
    ptx = [];
    pty = [];
  });
  $("#QCspinner").on("change", function () {
    varchange();
  });
...
function varupdate() {
  ...
  printcomment(
    "Mechanism will stop functioning when &theta; is out of bounds for given combination of linkages",
    1
  );
  printcomment(
    "AB=AC, OQ=OA <br> BPQC should form a Rhombus <br>A,Q,P are collinear",
    2
  );
  draw();
}
